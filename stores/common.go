...
func (gs *genericStore) setLimits(limits *StoreLimits) error {
	// Make a copy. This simply copy the PerChannel map pointer,
	// so we need a deep copy of that map.
	gs.limits = *limits
	sl := &gs.limits
	sl.PerChannel = make(map[string]*ChannelLimits, len(limits.PerChannel))
	for key, val := range limits.PerChannel {
		// Make a copy of the values too.
		sl.PerChannel[key] = &(*val)
	}
